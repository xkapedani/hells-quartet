...
function Drums({ onClose, resetOnOpen = false }) {
    ...
    async function playPattern(startDelay = 2000) {
        ...
        const clearId = setTimeout(() => {
            ...
            setActiveBeat(-1);
            setMessage(
                "Maintenant reproduis le rythme en cliquant sur le tambour ci-dessous.",
            );
        }, last);
    ...
    function playPatternWith(pat, startDelay = 2000) {
        ...
        const clearId = setTimeout(() => {
            ...
            setActiveBeat(-1);
            setMessage(
                "Maintenant reproduis le rythme en cliquant sur le tambour ci-dessous.s",
            );
        }, last);
        playingTimeoutsRef.current.push(clearId);
    }
    ...
    function stopRecording(passedClicks) {
        setRecording(false);
        setMessage("Évaluation en cours...");
    ...
    function onDrumClick() {
        ...
        if (!recording) {
            ...
            setRecording(true);
            setMessage(
                "Enregistrement... maintenant clique pour chaque battement afin de reproduire le rythme.",
            );
            // feedback for start
            PlayFromFile("one-time-drum.mp3");
    ...
    function evaluateAttempt(passedUserArray) {
        ...
        if (ok) {
            ...
            const hint = diffs.length
                ...
                : "";
            setMessage(
                "Pas tout à fait, réessaie ou écoute une fois de plus." + hint,
            );
